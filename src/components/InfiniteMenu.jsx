...
/*
 * Draggable sphere of image discs (React Bits, JS + CSS variant), WebGL2.
 *
 * Three changes from the upstream source, all needed because this is a SPA:
 *  - dispose(): the render loop re-queued itself forever, so every visit to the
 *    page left another rAF loop running. It now stops on unmount.
 *  - the constructor throws without a WebGL2 context; that would take the whole
 *    page down, so the caller gets a `fallback` instead.
 *  - `onItemClick` lets the host handle internal routes; upstream only opened
 *    external links and logged internal ones.
 */
...
class InfiniteGridMenu {
  ...
  // Stop the render loop and drop listeners. Without this the loop re-queues
  // itself forever and survives the component unmounting.
  dispose() {
    this.disposed = true
      ...
      this.items.map(
        (item) =>
          new Promise((resolve) => {
            ...
            // Keep the atlas build alive if one image 404s or is CORS-blocked.
            img.onerror = () => resolve(null)
            img.src = item.image
          })
      )
    ).then((images) => {
      // The component may have unmounted while the images were loading.
      if (this.disposed) return
...
/**
 * @param {object[]} items    { image, link, title, description }
 * @param {number}   scale    camera zoom
 * @param {Function} onItemClick  called with the active item when the arrow is
 *   pressed; use it for internal routes. Falls back to opening external links.
 * @param {React.ReactNode} fallback  rendered instead of the sphere when the
 *   browser has no WebGL2 context.
 */
export default function InfiniteMenu({ items = [], scale = 1.0, onItemClick, fallback = null }) {
  ...
  useEffect(() => {
    ...
    let sketch
    try {
      sketch = new InfiniteGridMenu(canvas, list, handleActiveItem, setIsMoving, (sk) => sk.run(), scale)
    } catch (err) {
      // No WebGL2 (older browser, blocklisted GPU, hardware acceleration off).
      console.warn('InfiniteMenu: falling back, no WebGL2 context.', err)
      setFailed(true)
      return
    }
      ...
      {activeItem && (
        <>
          ...
          <div
            ...
            role="button"
            tabIndex={0}
            aria-label={activeItem.title ? `Open ${activeItem.title}` : 'Open'}
            onKeyDown={(e) => {
